...
import { user, pet, petImages, swipes, matches, messages } from "./schema";
...
async function seed() {
  ...
  await db.transaction(async (tx) => {
    ...
    const [dijana] = await tx
      ...
      .returning({ id: user.id, email: user.email });
    console.log("Korisnik je kreiran:", dijana.email);
    ...
    const [rex] = await tx
      ...
      .values({
        vlasnikId: buki.id,
        ime: "Rex",
        opis: "Energican pas, voli trcanje i igru.",
        vrsta: "dog",
    ...
    const [luna] = await tx
      ...
      .values({
        vlasnikId: ana.id,
        ime: "Luna",
        opis: "Voli setnje i parkove. Drustven i miran pas.",
        vrsta: "dog",
    ...
    const [nora] = await tx
      ...
      .values({
        vlasnikId: marko.id,
        ime: "Nora",
        opis: "Mala mazna, voli paznju i igru.",
        vrsta: "dog",
    ...
    const [fluff] = await tx
      ...
      .values({
        vlasnikId: admin.id,
        ime: "Fluff",
        opis: "Macka koja obozava dremke i sunce.",
        vrsta: "cat",
    ...
    const [beki] = await tx
      ...
      .values({
        vlasnikId: dijana.id,
        ime: "Beki",
        opis: "Razigran pas koji voli setnje i drustvo.",
        vrsta: "dog",
    ...
    const [miki] = await tx
      ...
      .values({
        vlasnikId: milica.id,
        ime: "Miki",
        opis: "Veseo pas koji obozava lopticu i duge setnje.",
        vrsta: "dog",
    ...
    const [kiki] = await tx
      ...
      .values({
        vlasnikId: stefan.id,
        ime: "Kiki",
        opis: "Radoznala macka, voli da istrazuje i da se mazi kad se opusti.",
        vrsta: "cat",
    ...
    const [argo] = await tx
      ...
      .values({
        vlasnikId: ivana.id,
        ime: "Argo",
        opis: "Mirniji pas, super za drustvo i lagane setnje po kraju.",
        vrsta: "dog",
    ...
    const [mila] = await tx
      ...
      .values({
        vlasnikId: nikola.id,
        ime: "Mila",
        opis: "Druzeljubiva maca, voli paznju i da se igra kanapom.",
        vrsta: "cat",
    ...
    await tx.insert(petImages).values([
      ...
      { petId: tesla.id, url: "/pets/tesla.jpg", sortOrder: 0 },
    ]);
    ...
    await tx.insert(swipes).values([
      ...
      { fromPetId: nora.id, toPetId: luna.id, type: "like" },

      { fromPetId: nora.id, toPetId: fluff.id, type: "pass" },
      { fromPetId: fluff.id, toPetId: nora.id, type: "like" },
    ]);

    // 6) Auto-match: napravi matches za sve uzajamne like parove iz swipes
    ...
    // 7) Rucno dodavanje specificnih match-eva (bez pucanja)
    async function ensureMatch(a: string, b: string) {
      const [pet1Id, pet2Id] = sortPair(a, b);
    ...
    const matchId = await ensureMatch(nora.id, luna.id);
    await ensureMatch(rex.id, nora.id);

    // 8) Messages (vezano za match nora-luna)
    await tx.insert(messages).values([
      { matchId, senderPetId: luna.id, text: "Cao! Hocemo setnju u parku?" },
      { matchId, senderPetId: nora.id, text: "Moze! Kada ti odgovara?" },
