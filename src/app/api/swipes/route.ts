...
export async function POST(req: Request) {
  ...
  // upisi swipe (ako već postoji za isti par, update-uj type)
  await db
    .insert(swipes)
    .values({ fromPetId, toPetId, type })
    .onConflictDoUpdate({
      target: [swipes.fromPetId, swipes.toPetId],
      set: { type },
    });

  // ako je like, proveri obrnuti like → napravi match
  let matched = false;
    ...
    if (reverse) {
      ...
      if (!matchId) {
        const [existing] = await db
          ...
          .from(matches)
          .where(and(eq(matches.pet1Id, p1), eq(matches.pet2Id, p2)))
          .limit(1);
